...
import { VDSComponentSignal, VDSComponentInfo, SignalChange} from "../models/breadthSignals";
...
const VersionSelector: React.FC<VersionSelectorProps> = ({ recommendedChange }) => {
          ...
          <label className="px-3 flex flex-row items-center space-x-2">
            ...
            <div
              className={`border p-4 rounded-lg border-2 cursor-pointer px-6 py-2 rounded-full text-xl text-white ${
                version === "Major" ? "bg-black" : "bg-gray-300 "
              }`}
            >
              Major
            </div>
          </label>
          {recommendedChange === "Major" && (
        ...
        <div className="flex flex-col items-center">
          <label className="px-3 flex flex-row items-center space-x-2">
            <input
              ...
              className="form-radio h-3 w-3"
              id="minor"
              checked={version === "Minor"}
              onClick={() => handleRadioClick("Minor")}
            />
            <div
              className={`border p-4 rounded-lg border-2 cursor-pointer px-6 py-2 rounded-full text-xl text-white ${
                version === "Minor" ? "bg-black" : "bg-gray-300 "
              }`}
            >
              Minor
            </div>
        ...
        <div className="flex flex-col items-center">
          <label className="px-3 flex flex-row items-center space-x-2">
            <input
              ...
              className="form-radio h-3 w-3"
              id="patch"
              checked={version === "Patch"}
              onClick={() => handleRadioClick("Patch")}
            />
            <div
              className={`border p-4 rounded-lg border-2 cursor-pointer px-6 py-2 rounded-full text-xl text-white ${
                version === "Patch" ? "bg-black" : "bg-gray-300 "
              }`}
            >
              Patch
            </div>
      ...
      <ul className="text-lg mt-2 space-y-2 clear-both">
        <li className="flex flex-row bg-gray-100 p-2 rounded">
          ...
          <div className="text-black text-left">
            The color has changed, certain text may no longer be legible on this background
          </div>
        </li>
        <li className="flex flex-row bg-gray-100 p-2 rounded">
          <div className="text-black text-left text-3xl px-3 pt-2.5">•</div>
          <div className="text-black text-left">
            The border radius has changed, this may do something that looks bad somewhere
          </div>
        </li>
        <li className="flex flex-row bg-gray-100 p-2 rounded">
          <div className="text-black text-left text-3xl px-3 pt-2.5">•</div>
          <div className="text-black text-left">
            The drop shadow was added, this may produce strange effects on some designs
          </div>
        </li>
      </ul>
...
const VDSComponentDetail: React.FC<VDSComponentDetailProps> = ({
    ...
    const {name, icon, status, componentSignals, recomendedChange} = componentInfo;
    ...
    return (
      ...
      <VersionSelector recommendedChange={recomendedChange || "Major"} />
    </div>
  );
};
